...
| **Pure C (no `__asm`)** | **~41%** (1150 / 2829 functions) - contributions welcome |
| Hybrid (no `naked`, body still `__asm`) | ~10% (291 / 2829 functions) |
| Still `__declspec(naked)` | ~49% (1387 / 2829 functions) |
...
**The byte-match is done. The C decompilation is not.** About 1678
functions in `src/` (1387 naked + 291 hybrid) still have an
