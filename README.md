...
| **Pure C (no `__asm`)** | **~42%** (1186 / 2829 functions) - contributions welcome |
| Hybrid (no `naked`, body still `__asm`) | ~10% (285 / 2829 functions) |
| Still `__declspec(naked)` | ~48% (1357 / 2829 functions) |
...
**The byte-match is done. The C decompilation is not.** About 1642
functions in `src/` (1357 naked + 285 hybrid) still have an
