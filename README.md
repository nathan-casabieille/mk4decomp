...
| **Pure C (no `__asm`)** | **~40%** (1138 / 2829 functions) - contributions welcome |
| Hybrid (no `naked`, body still `__asm`) | ~10% (291 / 2829 functions) |
| Still `__declspec(naked)` | ~50% (1399 / 2829 functions) |
...
**The byte-match is done. The C decompilation is not.** About 1690
functions in `src/` (1399 naked + 291 hybrid) still have an
