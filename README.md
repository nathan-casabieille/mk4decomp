...
| **Pure C (no `__asm`)** | **~40%** (1139 / 2829 functions) - contributions welcome |
| Hybrid (no `naked`, body still `__asm`) | ~10% (291 / 2829 functions) |
| Still `__declspec(naked)` | ~50% (1398 / 2829 functions) |
...
**The byte-match is done. The C decompilation is not.** About 1689
functions in `src/` (1398 naked + 291 hybrid) still have an
