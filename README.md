...
| **Pure C (no `__asm`)** | **~40%** (1126 / 2829 functions) - contributions welcome |
| Hybrid (no `naked`, body still `__asm`) | ~10% (293 / 2829 functions) |
| Still `__declspec(naked)` | ~50% (1409 / 2829 functions) |
...
**The byte-match is done. The C decompilation is not.** About 1702
functions in `src/` (1409 naked + 293 hybrid) still have an
