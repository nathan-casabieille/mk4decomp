...
| **Pure C (no `__asm`)** | **~42%** (1188 / 2829 functions) - contributions welcome |
| Hybrid (no `naked`, body still `__asm`) | ~10% (285 / 2829 functions) |
| Still `__declspec(naked)` | ~48% (1355 / 2829 functions) |
...
**The byte-match is done. The C decompilation is not.** About 1640
functions in `src/` (1355 naked + 285 hybrid) still have an
