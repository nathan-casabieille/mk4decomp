...
| **Pure C (no `__asm`)** | **~40%** (1130 / 2829 functions) - contributions welcome |
| Hybrid (no `naked`, body still `__asm`) | ~10% (293 / 2829 functions) |
| Still `__declspec(naked)` | ~50% (1405 / 2829 functions) |
...
**The byte-match is done. The C decompilation is not.** About 1698
functions in `src/` (1405 naked + 293 hybrid) still have an
