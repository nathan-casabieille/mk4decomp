...
| **Pure C (no `__asm`)** | **~40%** (1127 / 2829 functions) - contributions welcome |
| Hybrid (no `naked`, body still `__asm`) | ~10% (293 / 2829 functions) |
| Still `__declspec(naked)` | ~50% (1408 / 2829 functions) |
...
**The byte-match is done. The C decompilation is not.** About 1701
functions in `src/` (1408 naked + 293 hybrid) still have an
