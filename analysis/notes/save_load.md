...
|------------|----------------------------|---------:|------|
| 0x004acce0 | `Config_RestoreGlobals`    |      376 | Copies the 588-byte buffer payload at `g_configBuffer + 0x0c` back into ~80 live in-memory settings. Lazy-init guard: if neither gate flag (`g_gsmFlag`, `g_configInitGate`) is set, sets both and calls `Config_SnapshotGlobals` first to seed the buffer from the current globals. Used as the 'apply settings' path from the menus. |
...
| `0x00543f7c`  | `g_configInitGate`          | Gate flag #1: set on first successful snapshot. While 0, Config_SnapshotGlobals bails. (Renamed from `g_audioRestoreSlot_00543f7c` - only ~20 of the ~80 snapshotted slots are audio-related, so the original name was misleading.) |
...
- **`g_configInitGate`** is the config-init gate, not an
