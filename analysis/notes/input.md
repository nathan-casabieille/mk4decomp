...
| 0x004b5850 | `Input_TickPlayers`                        |      448 | Per-frame input tick (was `GameStateMachineMaybeRebuild`). Clears the 4 action accumulators (`g_fightTableC0/C1/C2`, `g_phaseThunkInst`); runs keyboard + joystick polls for players 0 and 1; on a pending paused state checks ENTER/SPACE/ESC to release it; tail inverts each accumulator into the 'released' shadow used by edge-trigger code. |
...
|------------|-------------------------------------|---------|
| `0x00543b68` | `g_joySelP0`         | Selected joystick index for player 0 (-1 = no joystick bound). |
| `0x00543b6c` | `g_joySelP1`         | Selected joystick index for player 1 (-1 = no joystick bound). |
