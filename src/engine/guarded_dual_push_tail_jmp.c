...
#define g_dispatchSave22 (*(unsigned int *)MK4_VA(unsigned int, 0x535dacu))
...
/* NATIVE twin: the same body with the matrix-stack pushes through
 * the node seam. */
void GuardedDualPushTailJmp(void) {
    ...
    if (v != 0) {
        g_matrixStackTop++;
        *MK4_NODE(unsigned int, g_matrixStackTop) = g_eventQueueWorkType;
        g_matrixStackTop++;
        *MK4_NODE(unsigned int, g_matrixStackTop) = g_walkCallback;
        DownloadPlayerChar_Variant();
        if (g_framePauseFlag != 0) return;
        g_walkCallback = *MK4_NODE(unsigned int, g_matrixStackTop);
        g_matrixStackTop--;
        g_eventQueueWorkType = *MK4_NODE(unsigned int, g_matrixStackTop);
        g_matrixStackTop--;
    }
    TripleStateSwapCall();
}
