...
void DualEntryInstallSelfChain(void)

{
  MStackPushSet0008();
  if (g_framePauseFlag == 0) {
    g_walkCallback = 0x1008;
    MK4_NODE_AT(undefined4, g_baseSel, 0x74) = 0x1008;
    ScaledMove48to58();
    if (g_framePauseFlag == 0) {
      ArgSarStoreJmp(&(*(unsigned int *)MK4_VA(unsigned int, 0x4eef08)));
    }
  }
  return;
}
