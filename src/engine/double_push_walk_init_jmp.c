...
#define g_dispatchSave1303 (*(unsigned int *)MK4_VA(unsigned int, 0x4ece68u))
...
void DoublePushWalkInitJmp(void) {
    ...
    *(unsigned int *)(g_matrixStackTop * 4) = g_currentNodeIdx;
    ...
    g_matrixStackTop++;
#ifdef MK4_ARENA
    *MK4_NODE(unsigned int, g_matrixStackTop) = g_cj_0054205c;
#else
    *(unsigned int *)(g_matrixStackTop * 4) = g_cj_0054205c;
     ...
     * arena it yields a host address, not the VA (see MStackPackedInit) */
    g_currentNodeIdx = 0x004ece68u >> 2;
#else
