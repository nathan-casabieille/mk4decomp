/**
 * 6 "guarded push-call" wrappers.
 *
 * Pattern (28 bytes):
 *   call PreFunc                ; e8 + reloc
 *   mov eax, [g_framePauseFlag]
 *   test eax, eax
 *   jne short skip              ; 75 0d
 *   push ARG                     ; 68 + imm32
 *   call Target                 ; e8 + reloc
 *   add esp, 4
 * skip:
 *   ret
 */
#include "game/tick.h"   /* g_framePauseFlag */

/* Externs. */
extern void CopyJmp_GuardedChainPushSetCallPop_g_currentNodeIdx(void);
extern void Thunk_ScaledLoadInstallOrCall(void);
extern void ScaledLoadInstallOrCall_00433960(void);
extern int  PackedAdvanceCallTailJmp(int arg);
extern void CopyJmp_QuadFieldEarlyJmpThenInstall_g_currentNodeIdx(void);
extern int  ArgSarStoreJmp(int arg);
extern void Wrapper_ScaledChainPushCall_004ef920(void);
extern void DualMul10AndDispatchChain(void);
extern int  ArgSar_Set0_Jmp(int arg);

/* @addr 0x004338a0 */
void GuardedPushCall_ScaledLoadInstallOrCall_then_PackedAdvanceCallTailJmp(void) {
    ScaledLoadInstallOrCall_00433960();
    if (g_framePauseFlag) return;
    PackedAdvanceCallTailJmp(0x004e4480);
}

/* @addr 0x004338c0 */
void GuardedPushCall_Thunk_ScaledLoadInstallOrCall_then_PackedAdvanceCallTailJmp(void) {
    Thunk_ScaledLoadInstallOrCall();
    if (g_framePauseFlag) return;
    PackedAdvanceCallTailJmp(0x004e44a8);
}

/* @addr 0x0043b980 */
void GuardedPushCall_CopyJmp_then_ArgSarStoreJmp_0043b980(void) {
    CopyJmp_QuadFieldEarlyJmpThenInstall_g_currentNodeIdx();
    if (g_framePauseFlag) return;
    ArgSarStoreJmp(0x004e4c48);
}

/* @addr 0x0046a560 */
void GuardedPushCall_CopyJmp_then_ArgSarStoreJmp_0046a560(void) {
    CopyJmp_GuardedChainPushSetCallPop_g_currentNodeIdx();
    if (g_framePauseFlag) return;
    ArgSarStoreJmp(0x004eabc8);
}

/* @addr 0x00482990 */
void GuardedPushCall_Wrapper_ScaledChainPushCall_then_ArgSarStoreJmp(void) {
    Wrapper_ScaledChainPushCall_004ef920();
    if (g_framePauseFlag) return;
    ArgSarStoreJmp(0x004ee158);
}

/* @addr 0x0049c200 */
void GuardedPushCall_DualMul10AndDispatchChain_then_ArgSar_Set0_Jmp(void) {
    DualMul10AndDispatchChain();
    if (g_framePauseFlag) return;
    ArgSar_Set0_Jmp(0x004f2758);
}
