 ...
 *   B2: if g_dispatchVar15==1: tail-jmp DualScaledChainPush;
...
extern void DualScaledChainPush(void);
extern void MoveStackPipeline(void);

#ifdef NON_MATCHING
/* Ghidra-decompiled twin - behavior not yet runtime-verified */
void TripleBlockInstallSelf(void)
...
#else
__declspec(naked) void TripleBlockInstallSelf(void) {
    __asm {
        push    0x004ea948
        call    ArgSarStoreJmp
        add     esp, 4
        ...
        nop
        nop
        mov     eax, dword ptr [g_dispatchVar15]
        cmp     eax, 1
        mov     dword ptr [g_walkCallback], eax
        _emit   75h
        _emit   05h
        jmp     DualScaledChainPush
        push    0x004ea968
        call    ArgSarStoreJmp
        add     esp, 4
        ret
        ...
        mov     dword ptr [g_pendingNodeType], 0x0f
