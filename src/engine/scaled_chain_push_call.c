 ...
 *   eax = arg0>>2 → g_eventQueueTotal; ecx=[eax*4]; ++eax;
 *   g_walkCallback=ecx; g_eventQueueTotal=eax;
 *   call StorePauseImulShr16_004ab630; if pause: ret;
 *   eax=g_eventQueueTotal; ecx=g_walkCallback; eax+=ecx;
 *   g_eventQueueTotal=eax; eax=[eax*4]; g_walkCallback=eax;
 *   g_eventQueueCurrent=eax; eax&=0xffff; push eax;
 *   g_eventQueueWorkType=eax; call TaggedSceneDispatch_004be690; pop esp; ret.
 */
void ScaledChainPushCall_00489ee0(int arg0) {
    ...
    ((void (*)(int))TaggedSceneDispatch_004be690)((int)v);
}
