...
void ScaledArrStore_CallDualStoreXorBit(void) {
    ((ScenegraphNode *)MK4_PTR((g_fightGroupHead * 4)))->queue_end = (unsigned int)g_eventQueueIdx;
    CallDualStoreXorBit();
}

/* @addr 0x00428e70 - g_X[idx*4 + 0x28] = g_Y; jmp T */
void ScaledArrStore_EsiInstallBitCallChain_00428e70(void) {
    ((ScenegraphNode *)MK4_PTR((g_fightGroupHead * 4)))->queue_idx = (unsigned int)g_eventQueueCurrent;
...
void ScaledArrStore_ScaledChainJmp_00429450(void) {
    ((ScenegraphNode *)MK4_PTR((g_fightGroupHead * 4)))->queue_end = (unsigned int)g_xformEntityIdx;
...
void ScaledArrStore_GuardedChainCmpDualBitXor_00429980(void) {
    ((ScenegraphNode *)MK4_PTR((g_fightGroupHead * 4)))->queue_idx = (unsigned int)g_eventQueueChild;
    GuardedChainCmpDualBitXor();
}
