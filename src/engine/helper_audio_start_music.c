 ...
 *   the per-slot 0x1c-byte record at g_data_00f8fac8 (idx * 0x1c via
 *   lea/sub/shl ladder). Falls through if the slot table pointer is
 *   NULL or, when g_data_00f9efd4 is non-zero, the +0x16 flag bit 0 is
 ...
 *   writes (slot-id, cursor) into the 4-byte entries at g_data_00f9eb80
 *   indexed by free-slot. If two byte args [esp+0x18] and [esp+0x1c]
 *   are both <= 0x64, looks up a (arg1*25 + arg2 - based) offset in the
 *   table at g_data_00f85b60 and indirect-calls the vtables at +0x3c
...
__declspec(naked) void Helper_AudioStartMusic(void) {
    __asm {
        ...
        test    eax, eax
        je      L_tsa_failNeg
        mov     al, byte ptr [g_data_00f9efd4]
        test    al, al
    ...
    L_tsa_skipFlag:
        ...
        mov     cl, byte ptr [eax + ebp + g_data_00f8fadf]
        test    cl, cl
        jne     L_tsa_failNeg
        ...
        movsx   eax, cl
        shl     eax, 2
        mov     word ptr [eax + g_data_00f9eb80], si
        ...
        add     edi, ecx
        movsx   edx, dl
        mov     ecx, dword ptr [edi*4 + g_data_00f8fac8]
        lea     edi, [eax + eax*4]
        lea     edi, [edi + edi*4]
        lea     edi, [eax + edi*4]
        ...
        add     edi, edx
        shl     edi, 2
        movsx   edx, word ptr [edi + g_data_00f85b60]
        push    edx
        push    ecx
    ...
    L_tsa_skipPairCalls:
        ...
        sub     eax, ebx
        add     eax, ecx
        mov     cl, byte ptr [ebp + g_data_00f8fade]
        and     ecx, 1
        mov     eax, dword ptr [eax*4 + g_data_00f8fac8]
        push    ecx
        push    0
        push    0
    ...
    L_tsa_markCursor:
        ...
        mov     byte ptr [edx + ebp + g_data_00f8fadf], 1
        movsx   eax, word ptr [ebp + g_data_00f8fadc]
        inc     eax
        cdq
        xor     eax, edx
