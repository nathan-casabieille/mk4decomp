 ...
 *   call MStackPushTableWalk; if pause: ret;
 *   ecx = g_xformEntityIdx; eax = g_matrixStackTop;
...
void PushSetCallTailJmp(void) {
    ...
    *MK4_NODE(unsigned int, g_matrixStackTop) = g_xformEntityIdx;
    g_walkCallback = MK4_NODE_AT(unsigned int, g_cj_0054205c, 0x30);
#else
...
#ifdef MK4_ARENA
    g_walkCallback = g_xformEntityIdx;
