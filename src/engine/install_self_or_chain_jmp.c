 ...
 *   if ecx != 0: dec g_eventQueueChild;
 *     if zero: jmp CallPauseDirty1JmpDirty4StackPush_GuardedDoubleIncCmpJmp;
 *     else:    jmp CallPauseDirty4StackPushFn;
...
void InstallSelfOrChainJmp(void) {
    ...
    if (v != 0) {
        if (--g_eventQueueChild != 0) {
            CallPauseDirty4StackPushFn();
            return;
        }
        CallPauseDirty1JmpDirty4StackPush_GuardedDoubleIncCmpJmp();
        return;
    }
    *(unsigned int *)(base + 8) = MK4_CODE_VA(InstallSelfOrChainJmp);
