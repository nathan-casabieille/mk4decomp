 ...
 *   g_walkCallback=0x83; g_eventQueueCurrent=-1; call Thunk; pause? -> ret.
 *   esi=0; ebx=0x45; g_walkCallback=ebx; call MStackPush2LLWalkCompare; pause? -> ret.
 ...
 *     g_walkCallback=ebx=0x45; call MStackPush2LLWalkCompare;
 *     if (pause == 0): loop;
 */
...

{
  ...
  if (g_framePauseFlag == 0) {
    ...
    while ((g_framePauseFlag == 0 && (((byte)g_xformDirtyFlags & 4) == 0))) {
      MK4_NODE_AT(undefined4, (*(unsigned int *)MK4_VA(unsigned int, 0x542044)), 0x30) = 0x7a;
      MK4_NODE_AT(undefined4, (*(unsigned int *)MK4_VA(unsigned int, 0x542044)), 0x70) = 0xffffeb86;
      g_walkCallback = 0;
