...
/* @addr 0x0049c670 (91b)
 *   edx = g_fightAxisPosY; eax = g_baseSel;
 *   ecx = g_fightAxisPosX; push esi; esi = g_player1NodeIdx;
 *   g_eventQueueCurrent = edx; edx = g_cj_0054205c;
 *   g_walkCallback = ecx; eax <<= 2; cmp edx,esi;
 *   if eq: jmp store; ecx = g_fightAxisNegX; cmp edx,esi;
 *   g_walkCallback = ecx; if eq: jmp store;
 *   edx = g_fightAxisNegY; g_eventQueueCurrent = edx;
...
__declspec(naked) void CondPickDualStore(void) {
    __asm {
        ...
        _emit   74h
        _emit   0ch
        mov     edx, dword ptr [g_fightAxisNegY]
        mov     dword ptr [g_eventQueueCurrent], edx
