 ...
 *   g_eventQueueCurrent = edx; [eax*4 + 0x38] = ecx;
 *   eax = g_walkCallback; edx = g_eventQueueCurrent;
 *   eax -= edx; cmp eax, 0x1999;
 *   g_walkCallback = eax; if le: ret;
 *   else: jmp ChainWalkInstall_0049b7c0.
 */
void ChainStoreCmpJmp_0049b850(void) {
    ...
    *(unsigned int *)(v * 4 + 0x38) = 0xffffb334;
    delta = (unsigned int)g_walkCallback - g_eventQueueCurrent;
    g_walkCallback = (void (*)(void))delta;
    if ((int)delta > 0x1999) {
        ChainWalkInstall_0049b7c0();
    }
}
