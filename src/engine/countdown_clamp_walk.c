...
extern unsigned int g_walkCallback;
 ...
 * @addr 0x00422810 (97b)
 *
 * Two independent countdowns held in packed-pointer slots.
 *
 *   1. decrement [g_xformEntityIdx], floor it at 0, and mirror the value into
 *      both g_walkCallback and g_bootInitSaveSlot. `jns` skips the floor, so
 *      the clamp is on the SIGNED result.
 *   2. only when that first countdown has run out (<= 0), advance
 *      [g_currentNodeIdx] by 0xccc and ceiling it at 0x10000 - a 16.16 ramp
 *      reaching 1.0.
 *
 * The original stores g_walkCallback between the cmp and the jcc (MSVC's usual
 * scheduling); its order against the branch is immaterial here.
 *
...
void CountdownClampWalk(void)
{
    unsigned int idx = g_xformEntityIdx;
    int v = *MK4_NODE(int, idx) - 1;
