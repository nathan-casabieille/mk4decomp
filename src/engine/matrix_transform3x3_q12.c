...
/* Portable twin. A plain 3x3 by 3x3 product in Q12: the working matrix at
 * 0x007af990 times the caller's matrix, result into the caller's destination.
 * The original builds all nine elements in a stack scratch first and only then
 * copies them out, so a destination that overlaps the source still reads
 * pre-multiply values - `r` here is that scratch.
 *
 * BOTH arguments are host pointers. The original dereferences them directly
 * (`movsx ecx, word ptr [eax]`), even though every call site computes them with
 * `lea reg, [idx*4]` off a packed index. That is the opposite of the
 * convention the previous commit guessed at, and the two call sites are
 * corrected with it. */
void MatrixTransform3x3Q12(short *a, short *dst)
{
    const short *m = (const short *)MK4_VA(short, 0x007af990u);
    short r[9];
    int i, j;

    for (i = 0; i < 3; i++)
        for (j = 0; j < 3; j++)
            r[i * 3 + j] = (short)(((int)m[i * 3 + 0] * (int)a[0 * 3 + j]
                                  + (int)m[i * 3 + 1] * (int)a[1 * 3 + j]
                                  + (int)m[i * 3 + 2] * (int)a[2 * 3 + j]) >> 12);
...
#ifndef NON_MATCHING   /* native build takes src/engine/wt_snapshot_push_call.c */
