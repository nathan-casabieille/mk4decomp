...
/* @addr 0x0048ce00 (89b)
 *   Push g_scaledInit on mstack; call Helper_DownloadSetup;
 *   if pause: ret; eax = g_scaledInit; ecx = g_matrixStackTop;
...
void PushCallPopScaledJmpIndirect(void) {
    ...
    *(unsigned int *)MK4_PTR(top * 4) = g_currentNodeIdx;
    ...
    popped = *(unsigned int *)MK4_PTR(top * 4);
    top--;
    g_currentNodeIdx = popped;
    g_matrixStackTop = top;
    target = *(unsigned int *)MK4_PTR(saved_scaled * 4 + 8);
