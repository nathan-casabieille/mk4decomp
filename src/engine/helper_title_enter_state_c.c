...
extern void (__stdcall *g_iat_mciSendCommandA)(int, int, int, int);
extern int (__stdcall *g_iat_timeGetTime)(void);
void Helper_TitleEnterStateC(void) {
    if (g_timerFlag != 0 && Helper_AuxAudio_PostInit() != 0) {
        g_iat_mciSendCommandA((int)g_audioPreState, 0x808, 2, 0);
    }
    if (g_audioState00 == 0) {
        g_audioState00 = (unsigned int)g_iat_timeGetTime();
    }
    g_timerFlag = 0;
}
