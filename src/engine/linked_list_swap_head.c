...
#define g_arr_chain_4348f0_2c (*(unsigned int *)MK4_VA(unsigned int, 0x2cu))
#define g_arr_chain_4348f0_main (*(unsigned int *)MK4_VA(unsigned int, 0x0u))
#define g_audioBankSel (*(unsigned int *)MK4_VA(unsigned int, 0x537f94u))
#define g_audioBoundNode (*(unsigned int *)MK4_VA(unsigned int, 0x5437f0u))
#define g_baseSel (*(unsigned int *)MK4_VA(unsigned int, 0x542060u))
#define g_bootLongChainState (*(unsigned int *)MK4_VA(unsigned int, 0x537f24u))
...
/* Portable twin, written from the disassembly. If the node's +0x2c chain is
 * live: bracket g_xformEntityIdx, walk that chain to its LAST node, append the
 * old g_bootLongChainState list after it, and point g_bootLongChainState at
 * the node's chain - a splice of the whole chain onto the head of that list.
 * The walk leaves the cursor in g_xformEntityIdx as it goes, restored by the
 * bracket pop. */
void LinkedListSwapHead(void)
{
    ...
    *MK4_NODE(unsigned int, g_matrixStackTop) = g_xformEntityIdx;
    ...
    g_xformEntityIdx = cur;
    next = *MK4_NODE(unsigned int, cur);
    g_walkCallback = next;
    while (next != 0) {
        cur = next;
        g_xformEntityIdx = cur;
    ...
    g_walkCallback = g_bootLongChainState;
    *MK4_NODE(unsigned int, cur) = g_bootLongChainState;
    g_walkCallback = MK4_NODE_AT(unsigned int, g_currentNodeIdx, 0x2c);
    g_bootLongChainState = g_walkCallback;
