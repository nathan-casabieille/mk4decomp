...
/* @addr 0x00423720 (163b) - NATIVE ONLY; the matching build synthesizes it.
 *
 * The per-player frame entry the two fight handlers tail into. Clears the
 * scheduled node's four scratch slots (+0x80, +0x7c, +0x30, +0x4c), then
 * reads the fight group's own +0x30 - the player's state number. States 3
 * and 4 are the ones already installed, so they skip the phase reset;
 * anything else runs it first. Either way the state dispatch table runs,
 * and on a clean return the group's +0x24 keeps whatever the dispatch left
 * in the walk value while +0x28 is cleared. Pause exits leave the rest
 * alone. */
void ZeroSlotsGatedDispatch(void)
{
    unsigned int state;
    ...
    MK4_NODE_AT(unsigned int, g_baseSel, 0x30) = g_walkCallback;
    MK4_NODE_AT(unsigned int, g_baseSel, 0x4c) = g_walkCallback;

    state = MK4_NODE_AT(unsigned int, g_fightGroupHead, 0x30);
    g_walkCallback = state;
    if (state != 3 && state != 4) {
    ...
    MK4_NODE_AT(unsigned int, g_fightGroupHead, 0x24) = g_walkCallback;
    g_walkCallback = 0;
    MK4_NODE_AT(unsigned int, g_fightGroupHead, 0x28) = 0;
}
#else
/* no matching-side C - the synthesizer provides 0x00423720. */
#endif
