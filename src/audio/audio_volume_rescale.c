...
/* @addr 0x004ab690 (110b) - NATIVE ONLY; the matching build synthesizes it.
 *
 * Rescales the walk value by 1000/65536 - the triple lea-times-5 plus shl 3
 * is *125*8 - and compares it against what DualAddSar left in
 * g_dispatchSave404: below sets the walk's found bit, at-or-above clears it,
 * and only a FOUND result keeps the value; otherwise the walk slot is zeroed.
 * The paused exit skips all of it. */
void AudioVolumeRescale(void)
{
    unsigned int v;

    g_bootInitSaveSlot = g_walkCallback;
    g_dispatchSave404 = g_walkCallback;
    DualAddSar();
    if (g_framePauseFlag != 0)
        return;

    v = ((((g_walkCallback & 0xffffu) * 5u) * 5u) * 5u << 3) >> 16;
    g_walkCallback = v;
    if ((int)v < (int)g_dispatchSave404)
        g_xformDirtyFlags |= 1;
    else
...
#else
/* no matching-side C for AudioVolumeRescale - the synthesizer provides
 * 0x004ab690. */
#endif
