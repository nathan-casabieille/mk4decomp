...
#ifdef NON_MATCHING
/* no matching-side C here: the native twin lives in
 * src/game/config_snapshot_band.c, written against the disassembly and
 * addressed through MK4_VA. The Ghidra lift that used to sit here took
 * &g_audioStateGlobals as a HOST pointer, which the arena does not
 * accept, and was never runtime-verified. The empty branch stays so the
 * co-exec extractor still sees a twin pair. */
#else
