...
extern void ChainInitDoublePushCall(void);
extern void ClearTwoCallSetStore(void);
extern void CopyGlobal(void);
extern void DoublePushCallDualPopJmp(void);
...
extern void InstallChainInitTailJmp(void);
...
extern unsigned int g_audioPendByte619;
...
extern void IsNonzeroBoolFlagged(void);
extern void LoadGeoAsset_Default(void);
...
extern void TableWalkChainCall(void);
extern void TestByteOrAndCheck(void);
 ...
 *   ++g_bootInitState. esi=1. ecx=g_audioStateMachine0; walk g_audioBank2Byte2[i*24] for i in [0,ecx);
 *     if any !=0: keep esi=1; else esi=0. If esi: g_audioBankPick=2; tail to cleanup.
 *   Else: ebp=g_audioStateMachine1; esi=1. Walk g_audioPendByte61a[i*24] for i in [0,ebp). If esi: g_audioBankPick=1;
 ...
 *     If eax==2: roundrobin edx through ecx slots looking for g_audioBank2Byte2[edx*24]!=0; store to g_counter_0054359c.
 *     If eax==1: roundrobin edi through ebp slots looking for g_audioPendByte61a[edi*24]!=0; store to g_counter_005433c8.
...
__declspec(naked) void AudioBank2StatePickerWalk(void)
{
    __asm
    {
        ...
        lea     eax, [edi + edi*2]
        shl     eax, 3
        mov     byte ptr [eax + g_audioPendByte61a], 1
        mov     cl, byte ptr [eax + g_audioPendByte619]
        inc     cl
        mov     byte ptr [eax + g_audioPendByte619], cl
        jmp     short L_a92_afterStore
    L_a92_highBank:
        lea     eax, [edx + edx*2]
        shl     eax, 3
        mov     byte ptr [eax + g_audioBank2Byte2], 1
        mov     cl, byte ptr [eax + g_audioBank2Byte1]
        inc     cl
        mov     byte ptr [eax + g_audioBank2Byte1], cl
    L_a92_afterStore:
        ...
        mov     eax, offset g_audioBank2Byte2
    L_a92_walkHigh:
        cmp     byte ptr [eax], 0
    ...
    L_a92_lowBankCheck:
        ...
        mov     eax, offset g_audioPendByte61a
        mov     ecx, ebp
    L_a92_walkLow:
        cmp     byte ptr [eax], 0
    ...
    L_a92_rrHighSkip:
        lea     ecx, [edx + edx*2]
        cmp     byte ptr [ecx*8 + g_audioBank2Byte2], 0
    ...
    L_a92_rrLowSkip:
        lea     edx, [edi + edi*2]
        mov     al, byte ptr [edx*8 + g_audioPendByte61a]
        test    al, al
...
__declspec(naked) void Match_ChampionScreen(void)
{
    ...
    L_714c:
        ...
        call     MStackBracket3_FieldSequentialCopy
        cmp      dword ptr [g_framePauseFlag], edi
        jne      L_75fb
        mov      eax, dword ptr [g_currentNodeIdx]
        cmp      eax, edi
    ...
    L_72fd:
        ...
        lea      ecx, [eax + eax*2]
        lea      esi, [ecx*8 + g_audioBank2Base]
        jmp      L_7483
    L_746c:
        mov      edx, dword ptr [g_audioPreloadState]
        mov      eax, dword ptr [edx*4 + g_audioPreloadVar]
        lea      eax, [eax + eax*2]
        lea      esi, [eax*8 + g_audioBank2Base]
    L_7483:
        push     0x280000
        push     0x4d25a8
        ...
        shr      ecx, 2
        mov      dword ptr [g_currentNodeIdx], ecx
        call     ScaledSet1OnNonZero
        cmp      dword ptr [g_framePauseFlag], edi
        jne      L_75fb
        call     Phase1ChainAdvanceCallScale
        call     InstallChainInitTailJmp
...
__declspec(naked) void PendingMatch_Push16Call_004a3400(void)
{
    __asm {
        ...
        call     Push16Call
        call     AudioStateInitMultiInstall
        pop      edi
        pop      esi
        pop      ebp
        pop      ebx
...
__declspec(naked) void PendingMatch_MStackPush2ChainLLInsert_004a56c0(void)
{
    ...
    L_5e25:
        ...
        test     eax, eax
        jne      L_5e93
        call     MStackPushComplexCallPop_MStackPush2ChainPrepend_004064b0
        mov      eax, dword ptr [g_framePauseFlag]
        test     eax, eax
        jne      L_5e93
        mov      eax, dword ptr [g_currentNodeIdx]
