...
#ifdef NON_MATCHING
#include "portable/mem_model.h"
/* @addr 0x4ab790 - NATIVE twin. Pushes the eight scene-walk globals onto the matrix stack in one go -
 * node, entity, pending type, queue total and end, the record pointer,
 * the fight group and the base selector. ScenePostInitSequencer brackets
 * its whole build with this pair. */
void MStackPush8(void)
{
    g_matrixStackTop++;
    ...
    *MK4_NODE(unsigned int, g_matrixStackTop) = g_eventQueueEnd;
    g_matrixStackTop++;
    *MK4_NODE(unsigned int, g_matrixStackTop) = g_iatPtr_00542058;
    g_matrixStackTop++;
    *MK4_NODE(unsigned int, g_matrixStackTop) = g_fightGroupHead;
    g_matrixStackTop++;
    *MK4_NODE(unsigned int, g_matrixStackTop) = g_baseSel;
}
#else
