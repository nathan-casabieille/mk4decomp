...
/* @addr 0x004071a0 (395b) - NATIVE ONLY; the matching build synthesizes it.
 *
 * The slot-allocation driver the boot chain runs per node, under a two-value
 * bracket. The complex dispatcher picks the slot; a not-found THERE skips
 * everything and goes straight to the pops. Otherwise the node's +0x18 list
 * gets a fresh entry (marked in-use at +0x1c before the insert), the list is
 * re-walked, and BootStateTriple chases the record - a not-found from any of
 * those three lands in the failure path, the chain insert followed by
 * cur = fgh = 0. Success ORs 6 into byte 1 of the node's +0x20 and stores the
 * triple.
 *
 * The tail's bit-2 toggle tests the PATH'S register, not the popped value:
 * cur as the dispatcher left it when its own not-found skipped everything,
 * zero after the failure insert, fgh after a success. */
void BootDispatchSlotInit(void)
{
    unsigned int ok;
    ...
    *MK4_NODE(unsigned int, g_matrixStackTop) = g_pendingNodeType;
    ...
    if ((g_xformDirtyFlags & 4u) == 0) {
        unsigned int lst = MK4_NODE_AT(unsigned int, g_currentNodeIdx, 0x18);

        g_walkCallback = 1;
        g_xformEntityIdx = lst;
        MK4_NODE_AT(unsigned int, lst, 0x1c) = 1;
        ...
        if ((g_xformDirtyFlags & 4u) != 0) {
            ...
            MK4_NODE_AT(unsigned int, g_currentNodeIdx, 0x20) |= 0x600u;
...
#else
/* no matching-side C - the synthesizer provides 0x004071a0. */
#endif
