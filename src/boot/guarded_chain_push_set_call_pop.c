...
/* @addr 0x00406bb0 (94b) - NATIVE ONLY; there is no C source for this in the
 * matching build (the synthesizer takes it from the original bytes, and
 * symbols.yaml names this file as where it WOULD live).
 *
 * The guarded sibling of MStackPushZeroCallPop: if the node's +0x10 child is
 * null it does nothing; otherwise it brackets g_xformEntityIdx on the matrix
 * stack, walks the child list with selector 3, and pops - with the pop
 * SKIPPED on the paused exit, as every mstack bracket here leaks. */
void GuardedChainPushSetCallPop(void)
{
    g_walkCallback = MK4_NODE_AT(unsigned int, g_currentNodeIdx, 0x10);
    if (g_walkCallback == 0)
        return;
...
#else
/* no matching-side C - the synthesizer provides 0x00406bb0 from the original
 * bytes. The empty branch exists because the co-exec extractor recognises a
 * twin only as an ifdef/else pair. */
#endif
