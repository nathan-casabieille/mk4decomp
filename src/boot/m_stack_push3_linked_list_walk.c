...
/* @addr 0x004088b0 (268b) - NATIVE twin; the matching build synthesizes it.
 *
 * Bracketed list walk: saves node, entity and pending-type, then walks the
 * record list at node +0x1c - word 0 is the count, the elements follow.
 * Each positive element becomes the node for a ScaledTestPauseStore pass;
 * zero or negative elements are skipped. A not-found from the pass aborts
 * straight to the pops (success flag left set); walking out the count marks
 * success (callback 1, bit 2 cleared). The count word is do-while shaped:
 * the first element is taken even when the count starts at zero. Pause
 * exits leak the bracket. */
void MStackPush3LinkedListWalk(void)
{
    unsigned int w;
    ...
    *MK4_NODE(unsigned int, g_matrixStackTop) = g_pendingNodeType;
    ...
    g_pendingNodeType = MK4_NODE_AT(unsigned int, g_currentNodeIdx, 0x1c);
    g_eventQueueCurrent_mm = *MK4_NODE(unsigned int, g_pendingNodeType);
    g_pendingNodeType++;

    for (;;) {
        w = *MK4_NODE(unsigned int, g_pendingNodeType);
        g_walkCallback = w;
        g_pendingNodeType++;
        if ((int)w > 0) {
            g_currentNodeIdx = w;
            ...
            if (g_xformDirtyFlags & 4u) goto pops;  /* abort, flag kept */
        }
        g_eventQueueCurrent_mm--;
        if ((int)g_eventQueueCurrent_mm <= 0) break;
    }
    g_walkCallback = 1;
...
#else
/* no matching-side C - the synthesizer provides 0x004088b0. */
#endif
