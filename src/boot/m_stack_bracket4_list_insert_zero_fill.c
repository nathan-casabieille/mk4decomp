...
extern void ScaledLoadCallSet1c(void);
extern void MStackBracket2InitChainSplitInsert(void);
extern void PushSetCallCleanup(void);

/* @addr 0x00408600 (595b) - NATIVE twin; the matching build synthesizes it.
 *
 * The list installer. Saves the four-slot bracket, merges the node's
 * existing +0x1c list into the PVS if one is there (no pause check on that
 * call), then runs the set-1c pass over the +0x18 record with the element
 * counter cleared. A non-zero count means a list is wanted: a fresh block
 * of count+1 words comes from the chain-split insert, is zero-filled with
 * the count stamped into word 0, hooked onto +0x1c, and handed to the
 * cleanup pass. A not-found from the insert aborts to the pops. The tail
 * re-reads +0x1c of the RESTORED node and signals not-found only when that
 * came back null. Pause exits leak the bracket. */
void MStackBracket4_ListInsertZeroFill(void)
{
    unsigned int w, i, n;
    ...
    *MK4_NODE(unsigned int, g_matrixStackTop) = g_pendingNodeType;
    ...
    w = MK4_NODE_AT(unsigned int, g_xformEntityIdx, 0x1c);
    g_walkCallback = w;
    if (w != 0) {
        g_currentNodeIdx = w;
        PvsMergeDriver();                   /* no pause check, as original */
    }
    g_eventQueueCurrent_mm = 0;
    g_currentNodeIdx = MK4_NODE_AT(unsigned int, g_xformEntityIdx, 0x18);
    ScaledLoadCallSet1c();
    if (g_framePauseFlag != 0) return;      /* bracket leaked */

    w = g_eventQueueCurrent_mm;
    g_walkCallback = w;
    if (w != 0) {
        g_walkCallback = w + 1;
        g_matrixStackTop++;
        *MK4_NODE(unsigned int, g_matrixStackTop) = g_walkCallback;
        ...
        if (g_framePauseFlag != 0) return;  /* bracket leaked, 5 deep */
        MK4_NODE_AT(unsigned int, g_xformEntityIdx, 0x1c) = g_currentNodeIdx;
        g_xformLoopCounter = *MK4_NODE(unsigned int, g_matrixStackTop);
        g_matrixStackTop--;
        if ((g_xformDirtyFlags & 4u) == 0) {
            n = g_xformLoopCounter;
            g_walkCallback = 0;
            for (i = 0; i < n; i++)
                MK4_NODE_AT(unsigned int, g_currentNodeIdx, i * 4) =
                    g_walkCallback;
            g_xformLoopCounter = 0;
            *MK4_NODE(unsigned int, g_currentNodeIdx) = g_eventQueueCurrent_mm;
            g_currentNodeIdx = MK4_NODE_AT(unsigned int, g_xformEntityIdx, 0x18);
            g_xformEntityIdx = MK4_NODE_AT(unsigned int, g_xformEntityIdx, 0x1c);
            PushSetCallCleanup();
            if (g_framePauseFlag != 0) return;  /* bracket leaked */
        }
    }
    ...
    w = MK4_NODE_AT(unsigned int, g_currentNodeIdx, 0x1c);
    g_walkCallback = w;
    g_xformDirtyFlags |= 4;
    if (w != 0) g_xformDirtyFlags ^= 4;
}
#else
/* no matching-side C - the synthesizer provides 0x00408600. */
#endif
