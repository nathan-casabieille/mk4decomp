...
extern void DirtyPushCallPop(void);
extern void LinkedListInsert(void);
extern void LeaScaledCall(int);

/* @addr 0x00405e70 (580b) - NATIVE ONLY; the matching build synthesizes it.
 *
 * The recursive scene-tree teardown. For one node: optionally clear the slot
 * the node and g_fightGroupHead jointly name (both +0x1c fields positive),
 * detach the node - through the offset-4 unlink when its +0x14 link is live,
 * else by clearing g_fightGroupHead's +0x18 - then RECURSE over the child
 * list at node[0] until it drains (each child's own unlink is what removes it
 * from that list, so the loop re-reads the head every pass), swap the list
 * head if +0x2c says so, stash-and-clear +0x28 into g_bootInitSaveSlot and run
 * DirtyPushCallPop over what it held, insert the node into g_bootChainSlot2's
 * list, and drop the walk's found bit.
 *
 * Every paused exit skips the bracket pops, as all the mstack brackets here
 * leak. The dead second `cur == 0` test after the bit-2 toggle is the same
 * three-step shape MStackPush2ChainLLInsert has; kept as written. */
void MStackBracket2_TreeWalkRecursive(void)
{
    unsigned int d, c, save;
    ...
    *MK4_NODE(unsigned int, g_matrixStackTop) = g_pendingNodeType;
    ...
    if (g_currentNodeIdx != 0) {
        g_xformDirtyFlags ^= 4;
        if (g_currentNodeIdx == 0)
            goto pops;                        /* dead, as in the original */

        d = MK4_NODE_AT(unsigned int, g_currentNodeIdx, 0x1c);
        g_eventQueueCurrent = d;
        if ((int)d > 0) {
            g_walkCallback = g_fightGroupHead;
            if (g_fightGroupHead != 0) {
                c = MK4_NODE_AT(unsigned int, g_fightGroupHead, 0x1c);
                g_walkCallback = c;
                if (c != 0) {
                    g_walkCallback = 0;
                    g_xformEntityIdx = d + c;
                    *MK4_NODE(unsigned int, g_xformEntityIdx) = 0;
                }
            }
        }

        g_pendingNodeType = g_currentNodeIdx;
        c = MK4_NODE_AT(unsigned int, g_currentNodeIdx, 0x14);
        g_walkCallback = c;
        if (c != 0) {
            g_xformEntityIdx = 4;
            ...
            c = MK4_NODE_AT(unsigned int, g_currentNodeIdx, 0x18);
            g_walkCallback = c;
            if (c != 0) {
                g_walkCallback = 0;
                MK4_NODE_AT(unsigned int, g_fightGroupHead, 0x18) = 0;
            }
        }

        /* the child's own unlink is what advances this loop */
        c = *MK4_NODE(unsigned int, g_pendingNodeType);
        g_walkCallback = c;
        while (c != 0) {
            g_currentNodeIdx = c;
        ...
        c = MK4_NODE_AT(unsigned int, g_pendingNodeType, 0x2c);
        g_walkCallback = c;
        if (c != 0) {
            LinkedListSwapHead();
            if (g_framePauseFlag != 0)
        ...
        save = MK4_NODE_AT(unsigned int, g_currentNodeIdx, 0x28);
        g_bootInitSaveSlot = save;
        if (MK4_NODE_AT(unsigned int, g_currentNodeIdx, 0x28) != 0) {
            MK4_NODE_AT(unsigned int, g_currentNodeIdx, 0x28) = 0;
            save = g_bootInitSaveSlot;
        }
        g_xformDirtyFlags |= 4;
        g_currentNodeIdx = save;
        if (save != 0)
            g_xformDirtyFlags ^= 4;
        ...
        g_currentNodeIdx = g_pendingNodeType;
        g_xformEntityIdx = g_bootChainSlot2;
        LinkedListInsert();
        if (g_framePauseFlag != 0)
            return;
...
#else
/* no matching-side C - the synthesizer provides 0x00405e70; the empty branch
