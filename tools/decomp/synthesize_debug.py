            ...
            for r in sec['relocs']:
                if value <= r['va'] < value + target_size:
                    ...
                    if ref_va == 0 and ref_clean not in name_to_addr and ref_clean not in name_to_entry:
                        unresolved_reloc_offsets.append(offset_in_fn)
                        if 'DownloadPlayerChar' == clean:
                            import sys
                            print(f'  UNRESOLVED +0x{offset_in_fn:03x}: {ref_clean!r} (ref_sym.sec={ref_sym["sec"] if ref_sym else "None"})', file=sys.stderr)
                    elif 'DownloadPlayerChar' == clean and '$SG' in (ref_sym["name"] if ref_sym else ""):
                        import sys
                        print(f'  RESOLVED +0x{offset_in_fn:03x}: {ref_clean!r} -> ref_va=0x{ref_va:08x}', file=sys.stderr)
